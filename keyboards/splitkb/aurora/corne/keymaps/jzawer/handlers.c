...
//(OSX / WINDOWS) this variable is used to stablish the host OS. WINDOWS by default
enum OS os = WINDOWS;
        ...
        */
        ...
        case GAMELOCK:
            if(down){
                interrupt_mods();
                game_lock = !game_lock;
                layer_move(game_lock ? _GAME_MODE : _BASE );
            }
            return false;
